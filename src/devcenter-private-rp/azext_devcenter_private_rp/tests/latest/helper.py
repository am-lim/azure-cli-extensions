...
def create_dev_center(self):
    dev_center = self.cmd(
        "az devcenter-private-rp admin devcenter create "
        '--location "{location}" '
        '--tags CostCode="12345" '
        '--name "{devcenterName}" '
        '--resource-group "{rg}"'
...
def create_project_with_identity(self):
    ...
    project = self.cmd(
        "az devcenter-private-rp admin project create "
        '--location "{location}" '
        '--identity-type "UserAssigned" '
        '--user-assigned-identities "{{\\"{userAssignedIdentity}\\":{{}}}}" '
        '--name "{projectName}" '
...
def get_endpoint(self):
    project = self.cmd(
        "az devcenter-private-rp admin project show "
        '--name "{projectName}" '
        '--resource-group "{rg}"'
    ).get_output_in_json()
...
def create_network_connection(self):
    ...
    network_connection = self.cmd(
        "az devcenter-private-rp admin network-connection create "
        '--location "{location}" '
        '--tags CostCode="12345" '
...
def create_attached_network_dev_box_definition(self):
    ...
    self.cmd(
        "az devcenter-private-rp admin attached-network create "
    ...
    self.cmd(
        "az devcenter-private-rp admin devbox-definition create "
        '--dev-center "{devcenterName}" '
...
def create_env_type(self):
    ...
    self.cmd(
        "az devcenter-private-rp admin environment-type create "
        '--dev-center "{devcenterName}" '
        '--resource-group "{rg}" '
        '--name "{envTypeName}" '
...
def create_pool(self):
    ...
    self.cmd(
        "az devcenter-private-rp admin pool create "
    ...
    self.cmd(
        "az devcenter-private-rp admin schedule create "
        '--pool-name "{poolName}" '
        '--project-name "{projectName}" '
        '--resource-group "{rg}" '
        '--time "{time}" '
...
def catalog_create_and_sync_cmds(self):
    ...
    self.cmd(
        "az devcenter-private-rp admin catalog create "
        '--dev-center "{devcenterName}" '
        '--name "{catalogName}" '
    ...
    self.cmd(
        "az devcenter-private-rp admin catalog sync "
        '--dev-center "{devcenterName}" '
        '--name "{catalogName}" '
        '--resource-group "{rg}" '
    )
...
def create_proj_env_type(self):
    ...
    self.cmd(
        "az devcenter-private-rp admin project-environment-type create "
        '--project "{projectName}" '
